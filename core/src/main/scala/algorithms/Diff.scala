...
/** Represents the differentiation list of two sequences.
  * i.e. what to insert, delete, replace or keep to transform the first list into the second one.
  *
  * See [[Diff.apply]] et [[Diff.myers]] for more.
  */
enum Diff[+A, +B]:
  case Insertion[+B](value: B)             extends Diff[Nothing, B]
  case Deletion[+A](value: A)              extends Diff[A, Nothing]
  case Replacement[+A, +B](from: A, to: B) extends Diff[A, B]

  /** Represent identical elements. They can still be different but only considered identical.
    */
  case Identical[+A, +B](from: A, to: B) extends Diff[A, B]
...
object Diff:
  ...
  /** Compute the list of differences between sequences {{a1}} and {{a2}} by comparing elements via
    * the function {{eq}}
    *
    * Example: {{Diff((x:Char, y:Char) => x === y)("TABLE".toList, "PARBOILED".toList)}}
    *
    * to compute what to insert into, delete from, replace into or keep from "TABLE" to get
    * "PARBOILED"
    */
  @SuppressWarnings(
    Array(
  ...
  def apply[A, B](
            ...
            val l = List(
              ...
              choix(l1(j), 1, Insertion(c2)), {
                ...
                val (inc, mod): (Int, Diff[A, B]) =
                  if eq(c1, c2) then (0, Identical(c1, c2))
    ...
    d.foldLeft { init_l0(0)(Nil) } { case (s, (c2, j)) =>
      init_l0(j + 1)(Insertion(c2) :: s)
    }
          ...
          def auxK(k: Int): Option[(Int, D)] =
            ...
            then
              ...
              def furthest(x: Int, p: D): (Int, D) =
                if x < nl && y(x) < nr && eq(l(x), r(y(x))) then
                  furthest(x + 1, Identical(l(x), r(y(x))) :: p)
                else (x, p)

              val (x, p, suppr) =
                if k == -d || ((k != d) && v(k - 1)._1 < v(k + 1)._1) then
                  v(k + 1) match {
              ...
              then auxK(k + 2)
                ...
                val p2: D =
                  ...
                  then
                    if x > 0
                    then Deletion(l((x - 1))) :: p
                    else p
                  else if y(x) > 0
                  then Insertion(r((y(x) - 1))) :: p
                  else p

                val fxp = furthest(x, p2)

                if fxp._1 >= nl && y(fxp._1) >= nr
                then Some((d, fxp._2.reverse))
                else
                  v(k) = fxp
                  auxK(k + 2)
